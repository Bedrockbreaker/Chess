...
import { Gradient } from "../util/ColorSpline.ts";
import { getHexPos, isEqual } from "../util/Coords.ts";
import type { GameState, Move, Pos, TileOverlays, Tile as TileType } from "../util/types";
...
function Game() {
	...
	let selectedTile: TileType | undefined = undefined;
	for (let y = 0; y < height; y++) {
		for (let x = 0; x < width; x++) {

			const tile = gameState?.board[y][x];
			const pos = {x: x, y: y};
			const overlay = overlaysAtPos.get(getHexPos(pos));
			if (isEqual(selectedPos, pos) && tile?.piece) selectedTile = tile;
			...
			const selectMove = () => {
				...
				const moves = moveOptions.filter(halfTurn => halfTurn.some(move => {
					return selectedPos && (selectedPos.x !== x || selectedPos.y !== y)
						&& (isEqual(move.fromPos, pos)
						|| isEqual(move.toPos, pos)
						|| isEqual(move.removeAtPos, pos)
						|| isEqual(move.captureAtPos, pos)
						|| isEqual(move.spawnAtPos, pos)
						|| isEqual(move.dropAtPos, pos))
				}));
				if (moves.length > 1) {
					setSelectedMoveIndex(-1);
					return -1;
			...
			tiles.push(<Tile
				...
				onClick={() => {
					...
					if (((mobileStage === 3 && selectedMoveIndex > -1) || (mobileStage < 3 && selectedIndex === thisIndex && selectedIndex > -1)) && moveOptions) {
						socket.current?.send(`move ${JSON.stringify(moveOptions[selectedIndex].map(move => move.serialized))}`);
						setSelectedMoveIndex(-1);
					...
					// socket.current?.send(JSON.stringify({type: "moves", data: getHexPos(pos)}));
					socket.current?.send(`moves ${getHexPos(pos)}`);
					setSelectedPos(pos);
				}}
	...
	useLayoutEffect(() => {
		...
		onresize();
	}, [mobileStage, renderArea, width, height]);
	...
	const movesDiv = <div id="moves">{!gameState ? "Loading..." : (moveOptions ? moveOptions.map((halfturn, i) => {
		return <div
			...
			onClick={() => {
				...
				socket.current?.send(`move ${JSON.stringify(halfturn.map(move => move.serialized))}`);
				setSelectedMoveIndex(-1);
				setSelectedPos(undefined);
				setMoveOptions(undefined);
			}}
	...
	const tooltipDiv = selectedTile ? (<div id="tooltip">
		<div className="header">
			...
			<div className="internal">
				...
				<code>{getHexPos(selectedTile.pos)}</code>
			</div>
		</div>
